...
def create_student_report_for_day(report,student):
	...
	ts = daily_schedule.schedule.time_slots.filter(grade=report.course.grade,hour=report.course.hour).first()
...
class RetrieveTierTwoChartView(View):
	...
	def post(self,request,*args,**kwargs):
		...
		def get_goal_scores(goal,start,end):
			...
			hour_list = []
			for r in reports:
				if r.course.hour not in hour_list:
					hour_list.append(r.course.hour)
##			course_list = []
			##added 8/6/18
##			hour_list = ['Mentoring','1','2','3','5','6','7']
			for h in hour_list:
...
class RetrieveStudentStatisticsView(View):
	...
	def get(self,request,*args,**kwargs):
		...
		for b in bucks:
			course = Course.objects.filter(course_number=b.deposit.course_report.course.course_number,section_number=b.deposit.course_report.course.section_number).first()
